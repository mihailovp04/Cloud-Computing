...
### 9.2. Нагрузка через скрипт

### **Использование скрипта `curl.sh` для генерации нагрузки**

Для создания стабильной и контролируемой нагрузки на Auto Scaling Group используется нагрузочный скрипт **curl.sh**. Он отправляет множество параллельных запросов к `/load`, что позволяет быстро достичь высокого уровня CPU и вызвать автоматическое масштабирование.

#### **Пример запуска скрипта:**

```bash
./curl.sh <DNS_ALB> 30 60
```

Где:

* **<DNS_ALB>** — DNS-имя Application Load Balancer;
* **30** — количество параллельных потоков (число одновременных запросов);
* **60** — длительность нагрузки в секундах для одного запроса.

Скрипт используется для искусственного повышения нагрузки на EC2-инстансы, что позволяет наблюдать реакцию Auto Scaling Group и увидеть создание новых инстансов при превышении порога CPU.
...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <DNS_ALB> <threads> <duration_seconds>"
    exit 1
fi
...
echo "Target: http://$ALB_DNS/load?seconds=$DURATION"
echo "Threads: $THREADS"
echo "Duration per request: $DURATION sec"

for i in $(seq 1 $THREADS); do
    while true; do
        curl -s "http://$ALB_DNS/load?seconds=$DURATION" > /dev/null &
    done &
done
